...
class Scribe():
    ...
    def get_sample_of_len(self, length):
        ...
        at_wd = np.random.poisson(self.hbuffer) + self.hbuffer
    ...
    def get_char_of_index(self, index):
        try:
            return self.alphabet.chars[index]
        except Exception as e:
            print(len(self.alphabet.chars), index)
            raise e

    def data_generator(self):
        while True:
            x, y = self.get_sample()
            x = np.expand_dims(x.T, axis=(0, -1))
    ...
    def __repr__(self):
        if self.nchars_per_sample:
            ...
            cps = 'Depends on the length'
            if self.varying_len:
                len = 'Random, Avg:{}, Range:{}'.format(self.avg_len, self.len_range)
            else:
                len = self.avg_len
